...
import DescrContent from "./DescrContent";
import DescrLink from "./DescrLink";

import styled from "styled-components";

const StyledDescrTab = styled.div<{ $length: number }>`
  ${({ $length }) => `width: ${$length * 135}px`};
  height: 300px;
  margin: 0 auto;
...
const Description = () => {
  ...
  return (
    <StyledDescrTab $length={tabData.length}>
      <DescrLink
        currentTab={currentTab}
        data={tabData}
        handleTabData={handleTabData}
      />
      <DescrContent currentIndex={currentIndex} data={tabData} />
    </StyledDescrTab>
  );
};
